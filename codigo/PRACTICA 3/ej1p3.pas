...
writeln('el numero de socio menor es:', min^.dato.edad);
  ...
  function nro_max_edad(a: arbol):integer; //raroooooor
  var
  max, j: integer;
  begin 
  if (a <> nil) then begin
...
function esta (a: arbol; n: integer):boolean; // algo esta malS
begin
if (a = nil)  then 
 esta:= false
  else if (a^.dato.nro = n )then 
    esta:= true;
    if  (n < a^.dato.nro) then 
     esta:= esta (a^.hi, n)
       else esta:= esta (a^.hd, n);
     end;
     
function estaNOMBRE (a: arbol; nom: string):boolean; // algo esta malS
begin
if (a = nil)  then 
 estaNOMBRE:= false
...
begin
 if (a<>nil) then  begin
  cantSocios(a^.hi);
   suma:= suma + 1; // me apaerece que no se inicializo, por que????????????? fijarse la funcion de abajo 
   cantSocios(a^.hd)
   end
   else suma:= 0;
   end;
...
ok2:= estaNOMBRE(a, nombre);
 writeln('el valor leido ', ok2);//inciso vi
END.
